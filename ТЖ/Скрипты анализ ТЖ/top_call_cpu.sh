...
printf "%8s %10s %8s %8s %8s %8s %12s %8s %8s %s\n", "Cpu_sec", "sec", "avrg", "max", "cnt", "OutMB", "InMB", "MemPeakKB", "MemMB", "Context" \
; printf "%s\n" \
...
awk -v ShowBaseNameAWK=$ShowBaseName -v ShowBaseUsrNameAWK=$ShowBaseUsrName '{
 ...
 if(ToAdd==1)
 {
	...
	for (i in arr_dlits) {printf "%8d %10.2f %8.2f %8d %8d %8d %12d %8d %8d %s\n", arr_CpuTime[i], arr_dlits[i],
	arr_dlits[i] / arr_counts[i], dlit_max[i], arr_counts[i], arr_OutBytes[i], arr_InBytes[i], arr_MemoryPeak[i], arr_Memory[i], i}
