...
printf "%8s %8s %8s %8s %8s %8s %12s %8s %8s %s\n", "sec", "avrg", "max", "cnt", "OutMB", "InMB", "MemPeakKB", "MemMB", "Cpu_sec", "Context" \
; printf "%s\n" \
...
awk -v ShowBaseNameAWK=$ShowBaseName -v ShowBaseUsrNameAWK=$ShowBaseUsrName '{
 ...
 if(ToAdd==1)
 {
	...
	for (i in arr_dlits) {printf "%8d %8.2f %8.2f %8d %8d %8d %12d %8d %8d %s\n", arr_dlits[i],
	arr_dlits[i] / arr_counts[i], dlit_max[i], arr_counts[i], arr_OutBytes[i], arr_InBytes[i], arr_MemoryPeak[i], arr_Memory[i], arr_CpuTime[i], i}
