...
Перем КОМ_КОННЕКТОР; // "V83.COMConnector"
Перем ОжидатьВыполнениеОтложенныхОбработчиков;

Перем COMОбъект; // com-объект
...
Процедура Инициализация()
    ...
    ПрочитатьПараметр(Параметры, "-cluster1c_port", ПОРТ_КЛАСТЕРА_1С, 1545);
    ...
    //     Шаблон = НСтр("ru = 'Не удалось создать COM соединение:
    //             |%1'");
        
    //     ВызватьИсключение СтрШаблон(Шаблон, ТекстОшибки);
    // КонецПопытки;
...
Функция УстановитьВнешнееСоединениеСБазой(АутентификацияОперационнойСистемы = Ложь, ФайловыйВариантРаботы = Ложь)
    ...
    ШаблонСтрокиСоединения = "[СтрокаБазы][СтрокаАутентификации];UC=123456";
    
    Если ФайловыйВариантРаботы Тогда
        СтрокаБазы = "File = ""&КаталогИнформационнойБазы""";
        СтрокаБазы = СтрЗаменить(СтрокаБазы, "&КаталогИнформационнойБазы", ИМЯ_БАЗЫ_1С);
    Иначе
        СтрокаБазы = "Srvr = ""&ИмяСервера1СПредприятия""; Ref = ""&ИмяИнформационнойБазыНаСервере1СПредприятия""";
        СтрокаПорт = ?(ЗначениеЗаполнено(ПОРТ_КЛАСТЕРА_1С), ":" + ПОРТ_КЛАСТЕРА_1С, "");
        СтрокаБазы = СтрЗаменить(СтрокаБазы, "&ИмяСервера1СПредприятия", СЕРВЕР_1С + СтрокаПорт);
        СтрокаБазы = СтрЗаменить(СтрокаБазы, "&ИмяИнформационнойБазыНаСервере1СПредприятия", ИМЯ_БАЗЫ_1С);
    КонецЕсли;
    ...
    Исключение
        ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
        Шаблон = НСтр("ru = 'Не удалось подключится к другой программе:
                |%1, строка соединения %2'");
        ТекстСообщения = СтрШаблон(Шаблон, ТекстОшибки, СтрокаСоединения);
...
Процедура УстановитьБлокировкуСеансов()
    ...
    ПараметрыИБ.Вставить("ОкончаниеБлокировкиСеансов", ТекущаяДата() + ДлительностьБлокировки);
    ...
    Лог.Информация("Блокировка сеансов успешно");
...
Функция ОбновитьКонфигурациюИзХранилища()
    ...
    Исключение
        ...
        |%1'");
        ТекстОшибки = СтрШаблон(Шаблон, УправлениеКонфигуратором.ВыводКоманды());
...
Процедура ВыполнитьОбновлениеИнформационнойБазы()
    ...
    ВыполнитьОбновлениеКонфигурацииИнформационнойБазы();
