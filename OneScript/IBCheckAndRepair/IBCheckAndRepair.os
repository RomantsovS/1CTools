...
Процедура Инициализировать()
	...
	ПараметрыПроверки = ПараметрыПроверки + " -TimeLimit:3";

	ПользовательИБ = "CI_Automation";
	ПарольПользователяИБ = "";
	ИмяБазы = "ERP_TEST_FOR_CI_Template";
	ПараметрыПодключения = ПолучитьПараметрыПодключения(1, , "cv8app12:1741", "1740", ИмяБазы,
	Ложь, ПользовательИБ, ПарольПользователяИБ, "", "", , , "8.3.12",
	СтрШаблон("""\\rusklimat.ru\app\IT\1C_group\romantsov_s\OneScript\IBCheckAndRepair\out_%1_%2.txt""", ИмяБазы,
