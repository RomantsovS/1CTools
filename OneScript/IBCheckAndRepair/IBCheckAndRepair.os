...
Процедура Инициализировать()
	...
	ПараметрыПроверки = "";
	ПараметрыПроверки = ПараметрыПроверки + " -ReIndex";
	//ПараметрыПроверки = ПараметрыПроверки + " -LogIntegrity";
	//ПараметрыПроверки = ПараметрыПроверки + " -LogAndRefsIntegrity";
	ПараметрыПроверки = ПараметрыПроверки + " -RecalcTotals";
	ПараметрыПроверки = ПараметрыПроверки + " -Rebuild";
	ПараметрыПроверки = ПараметрыПроверки + " -IBCompression";
	ПараметрыПроверки = ПараметрыПроверки + " -ConfigurationExtensionsLogIntegrity";
	ПараметрыПроверки = ПараметрыПроверки + " -TestOnly";
	//ПараметрыПроверки = ПараметрыПроверки + " -UseStartPoint";
	ПараметрыПроверки = ПараметрыПроверки + " -TimeLimit:12:0";

	ПользовательИБ = "";
	ПарольПользователяИБ = "";
	ИмяБазы = "docmng_corp_romantsov";
	ПараметрыПодключения = ПолучитьПараметрыПодключения(1, , "hm-1c-dev", "1540", ИмяБазы,
	Ложь, ПользовательИБ, ПарольПользователяИБ, "", "", , , "8.3.19.1522",
	СтрШаблон("""t:\RomantsovSS\DocMng\out_%1_%2.txt""", ИмяБазы,
	Формат(ТекущаяДата(), "DF=yyyy-MM-dd-HH-mm-ss")), ПараметрыПроверки);
...
Функция ПолучитьПараметрыПодключения(ВариантРаботыИнформационнойБазы = 0, КаталогИнформационнойБазы = "", ИмяСервера1СПредприятия = "",
						...
						ПарольХранилища = "", ВерсияПлатформы = "83", ВерсияПлатформыСтрокой, ПустьКФайлуРезультата = "", КлючиПроверкиМодулей = "")
...
Функция ВыполнитьПроверкуКонфигурации(ПараметрыПодключения)
	...
	ПараметрыЗапуска.Добавить("/IBCheckAndRepair " + ПараметрыПодключения.КлючиПроверкиМодулей + " /Out " + ПараметрыПодключения.ПустьКФайлуРезультата);
	
	Попытка
		УправлениеКонфигуратором.ВыполнитьКоманду(ПараметрыЗапуска);
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Ошибка при IBCheckAndRepair.
			|%1'"), УправлениеКонфигуратором.ВыводКоманды()));
		Возврат Ложь;
	КонецПопытки;
	
	Лог.Информация(НСтр("ru = 'IBCheckAndRepair завершено.'"));
