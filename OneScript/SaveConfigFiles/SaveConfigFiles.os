...
Функция Инициализировать()
	...
	ВыгрузитьКонфигурациюВФайлы(ПараметрыПодключения, "w:\1C_group\romantsov_s\OneScript\SaveConfigFiles\src\", "w:\1C_group\romantsov_s\OneScript\SaveConfigFiles\list.txt");
...
// Обновляет конфигурацию из хранилища
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ВыгрузитьКонфигурациюВФайлы(ПараметрыПодключения, Каталог, ПутьКСпискуФайловВыгрузки)
	
	Лог.Информация(СтрШаблон(НСтр("ru = 'Выгрузка конфигурации в файлы %1, %2'"), Каталог, ПутьКСпискуФайловВыгрузки));
	...
	Попытка
		УправлениеКонфигуратором.ВыгрузитьКонфигурациюВФайлы(Каталог, , , , ПутьКСпискуФайловВыгрузки);
	Исключение
		Лог.Ошибка(СтрШаблон(НСтр("ru = 'Ошибка при выгрузке конфигурации в файловы.
			|%1'"), УправлениеКонфигуратором.ВыводКоманды()));
		Возврат Ложь;
	КонецПопытки;
	
	Лог.Отладка(НСтр("ru = 'Выгружено успешно.'"));
